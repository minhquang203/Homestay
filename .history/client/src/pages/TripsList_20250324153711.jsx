...
const TripsList = () => {
  ...
  const [invalidDataWarning, setInvalidDataWarning] = useState(null); // Thêm state để hiển thị cảnh báo
  ...
  // Đếm số lượng Booking không hợp lệ
  const invalidBookings = tripList.filter((trip) => !trip || !trip.listingId || !trip.hostId);

  useEffect(() => {
    if (invalidBookings.length > 0) {
      setInvalidDataWarning(
        `Có ${invalidBookings.length} đơn đặt phòng không thể hiển thị do dữ liệu không hợp lệ. Vui lòng liên hệ hỗ trợ.`
      );
    } else {
