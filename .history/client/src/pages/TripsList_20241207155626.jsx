...
const TripsList = () => {
  ...
  const cancelTrip = async (listingId) => {
    try {
      ...
      console.log(updatedTrip.message); // In ra thông báo hủy thành công
  
      // Xóa chuyến đi đã hủy khỏi danh sách tripList
      const updatedTripList = tripList.filter((trip) => trip.listingId._id !== listingId);
      dispatch(setTripList(updatedTripList)); // Cập nhật lại Redux store với danh sách mới
    } catch (err) {
