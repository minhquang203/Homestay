...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (token && message === "success") {
      const user = queryString.parse(window.location.search).user; // Lấy thông tin user từ query string nếu có
      dispatch(
        setLogin({
          user: user ? JSON.parse(user) : {}, // Lấy thông tin đầy đủ từ API
          token,
        })
      );
      navigate("/");
