  ...
  const cancelTrip = async (listingId) => {
    try {
      const response = await fetch(
        ...
        {
          method: "PATCH", // Sử dụng PATCH để cập nhật trạng thái chuyến đi
        }
      );
      ...
      // Cập nhật Redux để xóa chuyến đi đã hủy khỏi danh sách
      dispatch(
        setTripList((prevTrips) =>
          prevTrips.filter((trip) => trip.listingId._id !== listingId)
        )
      );
