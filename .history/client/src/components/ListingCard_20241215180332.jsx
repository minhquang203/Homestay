    ...
    if (booking) {
      try {
        ...
        const response = await fetch("http://localhost:3002/api/create_payment_url", {
          ...
          body: JSON.stringify({
            amount: totalPrice, // Giá trị thanh toán
            orderInfo: `Thanh toán chuyến đi từ ${startDate} đến ${endDate}`, // Mô tả đơn hàng
            orderType: "product", // Loại đơn hàng, có thể thay đổi tuỳ vào yêu cầu của bạn
            language: "vn", // Ngôn ngữ, có thể là "vn" hoặc "en"
          }),
        });
        ...
        if (data.paymentUrl) {
          ...
          navigate("/payment", { state: { paymentUrl: data.paymentUrl } });
        } else {
          // Nếu không tạo được URL thanh toán, thông báo lỗi
      ...
      // Nếu không có booking, chuyển hướng đến trang chi tiết property
