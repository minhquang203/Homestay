...
const ListingCard = ({
  ...
  const handleCardClick = async () => {
    if (booking) {
      try {
        // Đảm bảo dữ liệu cần thiết có sẵn (bookingId, amount, customerId)
        const bookingForm = {
          listingId: listingId, // Giả sử bạn đã có prop này
          amount: totalPrice, // Đây là tổng giá từ chuyến đi
          customerId: user._id, // Đây là ID khách hàng từ Redux
        };
        ...
        if (data.paymentUrl) {
          window.location.href = data.paymentUrl; // Chuyển hướng đến URL thanh toán VNPAY
        } else {
      ...
      navigate(`/properties/${listingId}`); // Nếu không có booking, điều hướng đến trang chi tiết bất động sản
    }
  };
