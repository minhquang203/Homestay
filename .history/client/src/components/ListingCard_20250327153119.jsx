...
const ListingCard = ({
  ...
  const handleCardClick = async () => {
    if (booking) {
      try {
        // Đảm bảo totalPrice và listingId được truyền từ props của ListingCard
        const amount = totalPrice; // Giả sử totalPrice là số tiền từ props
        const selectedBankCode = "NCB"; // Có thể để người dùng chọn ngân hàng, hoặc để trống
        ...
        const response = await fetch('http://localhost:3002/payment/create_payment_url', {
          ...
          body: JSON.stringify({
            ...
            bankCode: selectedBankCode, // Mã ngân hàng (có thể để trống)
          }),
        });
  
        // Kiểm tra nếu server trả về redirect
