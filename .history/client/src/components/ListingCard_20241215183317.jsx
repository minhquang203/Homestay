  ...
  const handleCardClick = async () => {
    if (booking) {
      try {
        ...
        if (!userId || !userName || !userEmail || !userPhone || !totalPrice) {
          setError("Thông tin không đầy đủ. Vui lòng kiểm tra lại.");
          return;
        }
  
        // Gửi yêu cầu đến server để tạo URL thanh toán
        const response = await fetch("http://localhost:3002/payment/create_payment_url", {
          ...
          body: JSON.stringify({
            ...
            orderInfo: `Thanh toán chuyến đi từ ${startDate} đến ${endDate}`, // Thông tin đơn hàng
            customerId: userId, // ID người đặt
            customerName: userName, // Tên người đặt
            customerEmail: userEmail, // Email người đặt
            customerPhone: userPhone, // Số điện thoại người đặt
          }),
        });
        ...
        if (data.paymentUrl) {
          // Chuyển hướng người dùng tới cổng thanh toán VNPay
