...
const PaymentPage = () => {
  ...
  const bookingData = location.state || {}; // ✅ Tránh lỗi khi không có state

  const [paymentMethod, setPaymentMethod] = useState('bank_transfer');
  const [message, setMessage] = useState('');

  // ✅ Kiểm tra dữ liệu có hợp lệ không, nếu không thì quay lại trang trước
  useEffect(() => {
    if (!bookingData.bookingId || !bookingData.totalPrice || !bookingData.customerId || !bookingData.hostId) {
  ...
  const handleSendMessage = async () => {
    try {
      const response = await fetch('http://localhost:3002/chat/send_message', {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          senderId: bookingData.customerId,
