...
router.delete("/:userId/trips/:listingId/cancel", async (req, res) => {
  try {
    const { userId, listingId } = req.params;

    // Tìm chuyến đi của người dùng
    const booking = await Booking.findOne({ customerId: userId, listingId });
    ...
    return res.status(200).json({ message: "Trip canceled successfully", booking });
  } catch (err) {
    console.log(err);
