...
router.delete('/users/:userId/trips/:listingId/cancel', async (req, res) => {
  const { userId, listingId } = req.params;
  console.log("Received DELETE request for userId:", userId, "listingId:", listingId);

  try {
    const trip = await Booking.findOne({ customerId: userId, "listingId._id": listingId });

    if (!trip) {
      return res.status(404).json({ message: "Trip not found" });
    }

    await trip.remove(); // Xóa chuyến đi khỏi database
