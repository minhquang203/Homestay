...
router.get(
  ...
  async (req, res) => { // Thêm từ khóa async vào đây
    try {
      const { token, user } = await generateGoogleLogin(req); // Đảm bảo generateGoogleLogin là một hàm async
      ...
      res.redirect("http://localhost:3000?message=error");
    }
  }
);
